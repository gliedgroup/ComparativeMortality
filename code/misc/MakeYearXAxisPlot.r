...
#All Countries Total
AllCountriesTotal <- JustTriple[c(3), c(1, 10:15, 17:22, 24:29, 31:36, 38:43, 45:50, 52:57)]
mAllCountriesTotal <- melt(AllCountriesTotal, id=c("Model"))
mAllCountriesTotal$year<- str_sub(mAllCountriesTotal$variable, start = -4)# Get Year
mAllCountriesTotal$agecat <- substring(mAllCountriesTotal$variable,10,14) # Get Age Cat
mAllCountriesTotal$agecat <- revalue(mAllCountriesTotal$agecat, c("80_Ye"= "80")) #Rename 80s

ggplot(data = mAllCountriesTotal, aes(x=year, y = value, group = agecat))+
  geom_line(aes(color = agecat))+
  geom_point(aes(color=agecat))+
